...
The **`sound-designer` skill** (`.claude/skills/sound-designer/`) is its **audio half** (the counterpart to art-direction): design + produce the music bed + per-scene sfx for a video, preferring **FREE** tools — the `synth` ffmpeg drone (`--music-provider synth`) and the local CC0 sfx library (`--sfx-provider local`, staged by `python scripts/fetch_sfx.py`) — over paid fal. Use it whenever a video feels silent/flat.
...
studio/
  ...
  providers/
    ...
    audio.py        sfx + music bed: fal-elevenlabs-sfx | fal-stable-audio | freesound(CC0) |
                    local (assets/audio/{sfx,music}/ CC0 lib, keyword-matched) | synth (FREE
                    ffmpeg drone) | silence. ffmpeg.synth_drone/synth_sfx generate beds+cues $0.
...
- **FREE sound layer (no library, no key): `--music-provider synth` + `--sfx-provider local`.** `ffmpeg.synth_drone` generates a $0 ambient drone bed from the mood string (cosmic/dread→deep minor; mournful→soft mid; folk→brighter major); `ffmpeg.synth_sfx` makes generic cues (whoosh/rumble/sparkle/impact/hum). `scripts/fetch_sfx.py` stages a CC0 sfx library into `assets/audio/sfx/` (synth starter always; Freesound CC0 recordings if `FREESOUND_API_KEY`). `assets/audio/` is gitignored (fetched, not committed; CC0 only — see its `CREDITS.md`). The **`sound-designer` skill** designs+produces the whole layer. ⚠️ Grain-free masters compress fine, but heavy `grain` fx still bloat video (separate gotcha) — sound has no such issue.
