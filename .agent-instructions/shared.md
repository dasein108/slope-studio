# Shared Agent Instructions — Slope Studio

Canonical project guidance for Codex, Claude Code, and other agents working in this repo. Tool-specific adapters must point here instead of duplicating these rules.
...
The **`film-maker` skill** (`.agent-instructions/skills/film-maker/`) is the operator playbook — invoke it when asked to produce, run, debug, or observe a video. It documents every stage's commands, providers, and how to inspect artifacts.

The **`sound-designer` skill** (`.agent-instructions/skills/sound-designer/`) is its **audio half** (the counterpart to art-direction): design + produce the music bed + per-scene sfx for a video, using **license-safe audio**: paid/generated providers we control (`fal-elevenlabs-sfx`, `fal-stable-audio`), synthetic ffmpeg audio, Freesound CC0/public-domain, or local assets with explicit reusable rights. Do not use copyrighted music, CC-BY/attribution-required tracks, CC-BY-NC/non-commercial audio, platform-restricted assets, or random community audio whose license is unclear. Do **not** put the same continuous drone under every video: choose a bed that matches the story, use silence/breathing room when better, and add small musical/SFX accents for dramatic or emotional beats.

The **`marketing-guru` skill** (`.agent-instructions/skills/marketing-guru/`) is the growth half: a closed, **self-improving ideate→deploy→measure→learn** loop that picks *what* to make and judges how viral it was. It's backed by a per-channel **journal** (`runs/_marketing/<channel>/journal.json`) and the `studio marketing` CLI sub-app. The loop is decomposed into **per-step lego-block skills** any agent can use alone — `marketing-ideate` (generate bets) · `marketing-deploy` (produce+publish+link) · `marketing-measure-learn` (score virality, then reflect into strategy) — plus **`marketing-autopilot`** (the hands-off scheduled driver, handling the 48–72h measurement-maturation wait). The umbrella **`marketing-guru`** also owns the thin read/pick/report helpers (journal state, backlog pick per the shipped bandit / 60-40 fallback, growth brief) and links the canonical memory model. The **thinking is agent-driven; the CLI commands are I/O helpers.** Deep reference + memory model: [`docs/50-marketing/`](docs/50-marketing/) (start at `README.md`, then `memory.md`, the canonical memory reference); why this shape: [`docs/20-research/self-improving-loop.md`](docs/20-research/self-improving-loop.md). Its **channel-setup lego-block** is the **`youtube-branding` skill** (`.agent-instructions/skills/youtube-branding/`) — `studio brand <spec.json>` generates a full brand kit (banner, avatar, transparent video-watermark logo, keywords, description) into `runs/_brand/<slug>/`.

The **`guerrilla-marketing` skill** (`.agent-instructions/skills/guerrilla-marketing/`) is the **indirect-reach half**: a `studio guerrilla` subsystem that posts a small, capped number of on-topic, **transcript-grounded** comments per day under other creators' recent videos, so readers who find a comment interesting click through to the channel. It is **gray-hat / ToS-adjacent**, so the design is built around **ban defense** — a single posting chokepoint enforces hard gates (age, comment count, channel size, 72h cooldown) and rails (deny-list, near-duplicate, spacing, active-window, blackout, style/opening diversity), and a **circuit breaker** (`studio guerrilla track`) auto-pauses on falling comment survival. The pipeline is `discover → rank → transcript → topic → highlight → compose → critic → rails → post`; `studio guerrilla tick` runs one cycle (`--dry-run` prints what it would post — read it before going live). Timestamp citation is **off by default**. It can run autonomously on a cron or be deployed to a VPS with clean internet via `scripts/remote/guerrilla.mk` (host/key in the gitignored `guerrilla.local.mk`). ⚠️ It posts to a **real channel** — never bypass the circuit breaker, ramp the daily cap slowly, and read a `--dry-run` batch before the first live post. Deep reference: [`docs/50-marketing/guerrilla/`](docs/50-marketing/guerrilla/).
...
studio/
  ...
  providers/
    ...
    audio.py        sfx + music bed: fal-elevenlabs-sfx | fal-stable-audio | freesound(CC0) |
                    local (assets/audio/{sfx,music}/ license-safe lib, keyword-matched) | synth (FREE
                    ffmpeg bed: drone/plucked/major pad) | silence. ffmpeg synth helpers generate beds+cues $0.
...
2. **Document + describe** — a page under `docs/30-animation/` (or `docs/30-animation/effects/`), and update the animator enum in `docs/30-animation/scenario-schema.md`, the decision matrix in `docs/30-animation/README.md`, this file's animator list, and `.agent-instructions/skills/film-maker/film-maker-guides.md` if it affects authoring.
...
- **`--max-cost` is the WHOLE-video budget (images + clips + music).** Paid/generated fal audio is allowed when it fits budget and story; if it does not fit, fall back to `local`/`freesound`/`synth` music and `local`/`freesound` SFX. Biggest levers remain hook clip length (10s $0.40 → 6s $0.24, or free `motion-*`) and using local/synth music beds. SFX should be sparse.
...
- **Music bed is ducked under narration** (`ffmpeg.duck_music`, sidechain). Default bed level is **−22.5 dB** (voice-forward but audible); raise/lower `music_db` to taste. The music track is faded in/out at the composition edges to avoid sharp starts/stops. The non-ducked `mux_audio` path uses a flat `volume=0.15` for music. Use music as arrangement, not wallpaper: fade in/out around the piece, leave breath where silence is stronger, and add accents at reveals, turns, tragedy, awe, or emotional payoff.
- **License-safe sound layer priority:** use paid/generated fal audio when quality and budget justify it; otherwise try **quality real music** from `local`/`freesound` when it is suitable and clearly reusable. Freesound imports must stay CC0/public-domain only. Use `synth` as the reliable no-library fallback; use `silence` when narration and visuals should breathe. `scripts/fetch_music.py` stages only Freesound CC0/public-domain music previews into `assets/audio/music/`. `synth` can generate $0 beds from the mood string (cosmic/dread→dark drone; mournful→soft minor bed; ancient/folk→plucked/lyre-like pulse; bright/hopeful→major pad). Do **not** use a generic drone by default for every video. `ffmpeg.synth_sfx` makes generic cues (whoosh/rumble/sparkle/impact/hum). `Scene.sfx` is for short accents: 0.2-2s normally, 5s absolute max, low gain under narration; long bells/drones/ambiences belong only as intentional environment or music. `scripts/fetch_sfx.py` stages a CC0 sfx library into `assets/audio/sfx/` (synth starter always; Freesound CC0 recordings if `FREESOUND_API_KEY`). `assets/audio/` is gitignored/fetched and licensing notes live in `assets/audio/CREDITS.md`. The **`sound-designer` skill** designs+produces the whole layer. ⚠️ Grain-free masters compress fine, but heavy `grain` fx still bloat video (separate gotcha) — sound has no such issue.
...
- **`parallax` = PERSPECTIVE / DEPTH scenery (mountains, clouds, houses, skyline) — depth planes drift at different speeds.** 🚫 **Do NOT author it on a frame a human / animal / face / single object DOMINATES** (takes most of the space): a big close subject has no depth to reveal and floats as a flat cutout — use `static` / `slice` / `motion-drift*` there. A *small* figure inside a wide landscape is fine (it's the nearest plane). Compose the still as foreground→midground→horizon, not a portrait. Layer-source priority (`animate._parallax`): (1) **two real plates** — held near scenery over a clean `_bg.png` plate → composited directly, **no inpaint, no tear** (balanced+ default; `studio visuals --parallax-plates`); else (2) **scenery** (`image_role:"bg"`) → a **sharp 2-plane DEPTH split** (`parallax_scenery`: two feathered depth bands pan oppositely at different speeds → real visible 2.5D, not a flat pan; never subject-inpainted). Direction from `motion_hint` (`right`/`left`/`up`/`down`). **Quality bar: aim for LAYERED depth — ≥2 clean planes; reach for 3 (hand-author far/mid/near PNGs; `parallax_drift` takes a per-plane `depth`) on the hero/establishing landscape.** **`blurred-parallax`** = the old soft look (blurred panning planes) — its 2-plane sky/ground mode is a free layered-depth shortcut for scenery. See `docs/30-animation/parallax.md`.
...
- **The quality playbook is `.agent-instructions/skills/film-maker/film-maker-guides.md`** — read it (and operator preferences) before authoring scenes for a video that must look great.
