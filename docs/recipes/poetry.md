...
(`duck_music`, default −22.5 dB, voice-forward). Raise/lower `music_db` to taste — for poetry keep
...
Do not put a generic drone under every poem; use silence, a real local track, or a sparse bed
with deliberate fade-in/fade-out and one restrained accent only where the line turns.

Audio rule: use only license-safe music and SFX. Paid/generated fal audio is allowed when budget
and story fit; local/Freesound community audio must be vetted reusable or CC0/public-domain. Then
use `synth`, then silence. Do not use attribution-required CC-BY, non-commercial CC-BY-NC,
copyrighted, platform-restricted, or unclear-license audio.
